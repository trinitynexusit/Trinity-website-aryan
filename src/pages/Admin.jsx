...
export default function Admin() {
  ...
  return (
    <main className="min-h-screen bg-[#0f172a] text-white pt-20 pb-20">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-10">
          {kpiCards.map((kpi) => {
            const Icon = kpi.icon;
            return (
              <div key={kpi.label} className="p-5 rounded-xl border border-white/8 bg-white/[0.02]">
                <div className="flex items-start justify-between mb-3">
                  ...
                  {kpi.up !== null && (
                    <div className={`flex items-center gap-1 text-xs font-medium ${kpi.up ? 'text-[#06b6d4]' : 'text-red-400'}`}>
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b border-white/5">
                    {['Name', 'Company', 'Domain Interest', 'Size', 'Status', 'Date', ''].map((col) => (
                      <th
                        key={col}
                        className="text-left text-xs font-semibold text-gray-500 uppercase tracking-widest px-5 py-4"
                      >
                        {col}
                      </th>
                    ))}
                  ...
                  {dummyLeads.map((lead, i) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <td className="px-5 py-4">
                          <span
                            className={`px-2.5 py-1 rounded-full text-xs font-medium border ${st.color} ${st.bg} ${st.border}`}
                          >
                            {lead.status}
                          </span>
                        </td>
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-3">
            {domains.map((domain) => {
              ...
              return (
                <div
                  key={domain}
                  className="p-4 rounded-xl border border-white/8 bg-white/[0.02] flex items-center justify-between group hover:border-[#06b6d4]/20 transition-colors"
                >
                  <div>
