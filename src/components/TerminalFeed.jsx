...
const LOG_TEMPLATES = [
    ...
    {
        type: 'RESOLVED', color: 'text-[#06b6d4]', messages: [
...
export default function TerminalFeed() {
    const [logs, setLogs] = useState(() => Array.from({ length: INITIAL_LOGS }, randomLog));
    ...
    return (<div className="rounded-xl border border-white/10 bg-[#020810] overflow-hidden font-mono text-xs shadow-2xl shadow-black/60">
      ...
      <div className="h-[400px] overflow-y-auto p-4 space-y-0.5 scrollbar-thin">
        {logs.map((log, i) => (<div key={log.id} className={`flex gap-2 items-start transition-all duration-300 ${i === logs.length - 1 ? 'animate-[fadeIn_0.4s_ease-out]' : ''}`}>
...
<span className="text-gray-600">
  events: <span className="text-[#00ff99] font-semibold drop-shadow-[0_0_6px_#00ff99]">4.2M/min</span> |
  analysts: <span className="text-[#00ff99] font-semibold drop-shadow-[0_0_6px_#00ff99]">12 online</span>
</span>
      </div>
    </div>);
}
